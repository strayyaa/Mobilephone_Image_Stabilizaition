...
if (-not (Test-Path "./logs/LongForecasting")) { New-Item -ItemType Directory -Path "./logs/LongForecasting" | Out-Null }
...
$model_id_name = "AngleDataFreqMoE"
$data_name = "angle"
$target = "z_angle_1"
...
Write-Host "Starting PatchTST training with freqMoE-enhanced inputs..." -ForegroundColor Green
...
foreach ($pred_len in $pred_lengths) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ freqMoE training completed: $log_file" -ForegroundColor Green
    } else {
        Write-Host "❌ freqMoE training failed: $log_file" -ForegroundColor Red
    }
}
