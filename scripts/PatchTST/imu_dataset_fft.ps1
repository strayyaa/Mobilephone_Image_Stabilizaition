...
Write-Host "Starting PatchTST training on FFT-enhanced IMU data..." -ForegroundColor Green
...
foreach ($pred_len in $pred_lengths) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ FFT training completed: $log_file" -ForegroundColor Green
    } else {
        Write-Host "❌ FFT training failed: $log_file" -ForegroundColor Red
    }
}
